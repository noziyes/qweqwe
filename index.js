...
fs.readdir("./src/commands", (err, files) => {
    ...
    console.log(`[DBotList.ml]: ${files.length} command loaded.`);
    files.forEach(f => {
...
client.on('ready',async () => {
    console.log("[DBotList.ml]: Bot successfully connected as "+client.user.tag+".");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'DBotList.ml | '+bots.length+' bots' }, status: "online" });
});
/*=======================================================================================*/
