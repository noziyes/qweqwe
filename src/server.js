  ...
  module.exports = async (client) => {
    ...
    app.engine("dbotlist-ml", ejs.renderFile);
    app.set("view engine", "dbotlist-ml");
   ...
   app.get("/login", (req, res, next) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://dbotlist.ml/user/${a.id}) The user named **site** tried to log in but could not log in because he was blocked from the site. `).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
        })
        req.session.destroy(() => {
        res.json({ login: false, message: "You have been blocked from DBotList.", logout: true })
        req.logout();
        });
        } else {
            try {
        ...
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://dbotlist.ml/user/${a.id}) User named **site** logged in.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    http.listen(3000, () => { console.log("[DBotList.ml]: Website running on 3000 port.")});
...
app.post("/admin/maintence", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(global.config.server.channels.webstatus).send(`<a:Dis_off:850922384080240640> DBotList has been switched to __Maintenance__ due to **${req.body.reason}** [||<@&844914717377691678>||]`).then(a => {
        new maintenceSchema({
            server: config.server.id,
...
app.post("/admin/unmaintence", global.checkAuth, async (req, res) => {
    ...
    const bakimsonaerdikardesvcodes = new dc.MessageEmbed()
        .setAuthor("dbotlist.ml", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`<a:Dis_on:850922176718045185> DBotList are **active** again!\n[Click to redirect website](https://dbotlist.ml)`)
        .setFooter("DBotList © All rights reserved.");
    ...
    sleep(1050)
    console.clear();
    console.log('\x1b[36m%s\x1b[0m', "[DBotList.ml]: General routers loading...");
    sleep(500);
    ...
    /* Uptime System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[DBotList.ml]: Uptime system routers loading...");
    sleep(500);
    ...
    /* Profile System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[DBotList.ml]: Profile system routers loading...");
    sleep(500);
    ...
    /* Code Share System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[DBotList.ml]: Code Share system routers loading...");
    sleep(500);
    ...
    /* Botlist System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[DBotList.ml]: Botlist system routers loading...");
    sleep(500);
    ...
    /* Server List System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[DBotList.ml]: Serverlist system routers loading...");
    sleep(500);
    ...
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[DBotList.ml]: Admin Panel system routers loading...");
    sleep(500);
    ...
    /* Bot System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[DBotList.ml]: Bot system loading...");
