...
console.log("[DBotList.ml]: Admin/Botlist/Decline Bot router loaded.");
...
app.post("/admin/certificate/delete/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<:unverified:845736611479945226> <@${botdata.ownerID}>'s bot named **${bota.tag}** has not been granted a certificate.`)
        client.users.cache.get(botdata.ownerID).send(`<:unverified:845736611479945226> Your bot named **${bota.tag}** certificate application has been declined.\nReason: **${rBody['reason']}**`)
    });
