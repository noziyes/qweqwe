...
console.log("[DBotList.ml]: Admin/Botlist/Certificate Give router loaded.");
...
app.get("/admin/certificate/give/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<:verify:845717753884442634> <@${botdata.ownerID}>'s bot named **${bota.tag}** has been granted a certificate by <@${req.user.id}>`)
