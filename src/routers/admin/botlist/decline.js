...
app.post("/admin/decline/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.ownerID).then(sahip => {
        client.channels.cache.get(channels.botlog).send(`<:Blurple_cross:853005850234978315> <@${botdata.ownerID}>'s bot \`${botdata.username}\` has been declined by <@${req.user.id}>`)
        client.users.cache.get(botdata.ownerID).send(`<:Blurple_cross:853005850234978315> bot named **${botdata.username}** has been declined.\nReason: **${rBody['reason']}**\nAuthorized: **${req.user.username}**`)
    })
