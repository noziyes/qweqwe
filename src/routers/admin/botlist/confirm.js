...
console.log("[DBotList.ml]: Admin/Botlist/Confirm Bot router loaded.");
...
app.get("/admin/confirm/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.params.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<:blurple_check:915978290743894027>  <@${botdata.ownerID}>'s Bot <@${req.params.botID}> Has Been Approved by <@${req.user.id}>`)
        client.users.cache.get(botdata.ownerID).send(`<:blurple_check:915978290743894027> bot **${bota.tag}** has been approved by ${req.user.username}`)
    });
