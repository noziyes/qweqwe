...
console.log("[DBotList.ml]: Botlist/Edit router loaded.");
...
app.post("/bot/:botID/edit", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.params.botID).then(a => {
        client.channels.cache.get(channels.botlog).send(`<:edited:916992596168024104> <@${req.user.id}> edited <@${req.params.botID}>`)
