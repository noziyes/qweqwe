...
fs.readdir("./src/servers/commands/", (err, files) => {
  ...
  console.log(`[DBotList.ml/servers]: ${files.length} command loaded.`);
...
serverClient.on('ready',async () => {
    console.log("[DBotList.ml/servers]: Bot successfully connected as "+serverClient.user.tag+".");
    serverClient.user.setPresence({ activity: { type: 'WATCHING', name: '+bump & +vote | DBotList.ml/servers' }, status: "online" });
});
